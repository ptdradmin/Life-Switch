...
-- Storage bucket for secret media
INSERT INTO storage.buckets (id, name, public) VALUES ('secret-media', 'secret-media', false);

CREATE POLICY "Users can upload own media" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'secret-media' AND auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY "Users can view own media" ON storage.objects FOR SELECT USING (bucket_id = 'secret-media' AND auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY "Users can delete own media" ON storage.objects FOR DELETE USING (bucket_id = 'secret-media' AND auth.uid()::text = (storage.foldername(name))[1]);
